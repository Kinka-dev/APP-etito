// app/add.tsx
import { nutritionIndex, nutritionList } from "@/app/data/nutritionIndex";
...
const fuse = new Fuse(nutritionList, {
  threshold: 0.3,
  includeScore: true,
});
...
export default function AddRecipeScreen() {
  ...
  const [selectedIngredientForLink, setSelectedIngredientForLink] = useState<
    number | null
  >(null);
  ...
  const ingredientsInGroup = (groupId: string) => {
    return ingredients.filter(
      (i) => typeof i.groupId === "string" && i.groupId === groupId,
    );
  };
  ...
  const isGroupTitleTaken = (title: string) => {
    return groups.some(
  ...
  // ---------------------- ORDINAMENTO INGREDIENTI LIBERI ----------------------
  ...
  const handleSave = () => {
    ...
    const enrichedIngredients = ingredients.map((ing) =>
      ing.type === "ingredient" ? applyNutrition(ing) : ing,
    );

    // 2️⃣ Ingredienti raggruppati
    const finalIngredients = groupsList.map((g) => ({
      id: g.id,
      title: g.title,
      items: ingredients
        .filter(
    ...
    // 3️⃣ Ingredienti liberi
    const freeIngredients = ingredients
      .filter(
        (i) =>
          i.type === "ingredient" && i.groupId === null && i.name.trim() !== "",
      )
      .map((ing) => ({
        id: ing.id,
        name: ing.name,
                          ...
                          <View style={{ flex: 1 }}>
                            <Input
                              ...
                              onChangeText={(t) => {
                                ...
                                if (t.length > 1) {
                                  const results = fuse.search(t).slice(0, 5);
          ...
          >
            ...
            >
              <Ionicons
                ...
                color={COLORS.textLight}
              />
              <Text style={{ color: COLORS.textLight, fontSize: 10 }}>OCR</Text>
            </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  fab: {
    ...
    shadowOffset: { width: 0, height: 4 },
    elevation: 10,
    zIndex: 9999, // ⭐ fondamentale
  },

  /* ============================================================
   CARD ESPANSA PREMIUM
============================================================ */
  /* MINI CARD */
  stepCollapsedCard: {
    flexDirection: "row",
  ...
  stepCollapsedNumber: {
    position: "absolute",
    top: 0, // ⭐ spostato dentro
    left: 0, // ⭐ spostato dentro
    width: 28, // leggermente più grande
    height: 28,
    borderRadius: 14,
    backgroundColor: COLORS.secondary,
    justifyContent: "center",
  ...
  stepNumberExpanded: {
    position: "absolute",
    top: 15, // ⭐ spostato dentro
    left: 5, // ⭐ spostato dentro
    width: 32, // leggermente più grande
    height: 32,
    borderRadius: 20,
    backgroundColor: COLORS.secondary,
    justifyContent: "center",
