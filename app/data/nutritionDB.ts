export const cereali = {
  "farina 00": {
    kcal: 364,
    carbs: 76.0,
    protein: 10.0,
    fat: 1.0,
  },
  "farina integrale": {
    kcal: 340,
    carbs: 68.0,
    protein: 13.0,
    fat: 2.5,
  },
  "farina di mais": {
    kcal: 365,
    carbs: 76.0,
    protein: 7.0,
    fat: 3.5,
  },
  "farina di riso": {
    kcal: 366,
    carbs: 80.0,
    protein: 6.0,
    fat: 0.7,
  },
  "farina di avena": {
    kcal: 389,
    carbs: 66.0,
    protein: 17.0,
    fat: 7.0,
  },
  "farina di grano duro": {
    kcal: 339,
    carbs: 71.0,
    protein: 12.5,
    fat: 1.5,
  },
  "farina di castagne": {
    kcal: 369,
    carbs: 74.0,
    protein: 6.0,
    fat: 3.0,
  },
  "farina di mandorle": {
    kcal: 590,
    carbs: 19.0,
    protein: 21.0,
    fat: 50.0,
  },
  "farina di cocco": {
    kcal: 660,
    carbs: 24.0,
    protein: 7.0,
    fat: 64.0,
  },
  "amido di mais": {
    kcal: 381,
    carbs: 91.0,
    protein: 0.3,
    fat: 0.1,
  },
  "amido di riso": {
    kcal: 381,
    carbs: 90.0,
    protein: 0.5,
    fat: 0.1,
  },
  "fiocchi d'avena": {
    kcal: 389,
    carbs: 66.0,
    protein: 17.0,
    fat: 7.0,
  },
  "corn flakes": {
    kcal: 357,
    carbs: 84.0,
    protein: 8.0,
    fat: 0.4,
  },
  muesli: {
    kcal: 389,
    carbs: 69.0,
    protein: 11.0,
    fat: 7.0,
  },
  quinoa: {
    kcal: 368,
    carbs: 64.0,
    protein: 14.0,
    fat: 6.0,
  },
  bulgur: {
    kcal: 342,
    carbs: 76.0,
    protein: 12.0,
    fat: 1.3,
  },
  "cous cous": {
    kcal: 376,
    carbs: 77.0,
    protein: 13.0,
    fat: 0.6,
  },
  orzo: {
    kcal: 354,
    carbs: 73.0,
    protein: 12.0,
    fat: 2.3,
  },
  farro: {
    kcal: 335,
    carbs: 67.0,
    protein: 15.0,
    fat: 2.5,
  },
  "grano saraceno": {
    kcal: 343,
    carbs: 71.0,
    protein: 13.0,
    fat: 3.4,
  },
  miglio: {
    kcal: 378,
    carbs: 73.0,
    protein: 11.0,
    fat: 4.2,
  },
  segale: {
    kcal: 338,
    carbs: 75.0,
    protein: 10.0,
    fat: 1.7,
  },
  sorgo: {
    kcal: 329,
    carbs: 72.0,
    protein: 11.0,
    fat: 3.3,
  },
  amaranto: {
    kcal: 371,
    carbs: 65.0,
    protein: 14.0,
    fat: 7.0,
  },
  crackers: {
    kcal: 421,
    carbs: 72.0,
    protein: 10.0,
    fat: 9.0,
  },
  "gallette di riso": {
    kcal: 387,
    carbs: 82.0,
    protein: 8.0,
    fat: 2.8,
  },
  "gallette di mais": {
    kcal: 381,
    carbs: 80.0,
    protein: 7.0,
    fat: 3.0,
  },
  popcorn: {
    kcal: 387,
    carbs: 78.0,
    protein: 12.0,
    fat: 4.5,
  },
  "cereali soffiati": {
    kcal: 380,
    carbs: 85.0,
    protein: 7.0,
    fat: 1.0,
  },
  granola: {
    kcal: 489,
    carbs: 64.0,
    protein: 10.0,
    fat: 24.0,
  },
  "puffed rice": {
    kcal: 383,
    carbs: 87.0,
    protein: 6.0,
    fat: 1.0,
  },
  "puffed wheat": {
    kcal: 367,
    carbs: 78.0,
    protein: 12.0,
    fat: 2.0,
  },
  "fiocchi di mais": {
    kcal: 357,
    carbs: 84.0,
    protein: 8.0,
    fat: 0.4,
  },
  "fiocchi di farro": {
    kcal: 350,
    carbs: 67.0,
    protein: 14.0,
    fat: 2.5,
  },
  "fiocchi di avena integrali": {
    kcal: 389,
    carbs: 66.0,
    protein: 17.0,
    fat: 7.0,
  },
  "fiocchi di segale": {
    kcal: 338,
    carbs: 75.0,
    protein: 10.0,
    fat: 1.7,
  },
  "fiocchi di quinoa": {
    kcal: 368,
    carbs: 64.0,
    protein: 14.0,
    fat: 6.0,
  },
  "fiocchi di miglio": {
    kcal: 378,
    carbs: 73.0,
    protein: 11.0,
    fat: 4.2,
  },
  "fiocchi di grano": {
    kcal: 340,
    carbs: 72.0,
    protein: 12.0,
    fat: 2.0,
  },
};

export const pane_pizza_prodotti_da_forno = {
  "pane bianco": {
    kcal: 265,
    carbs: 49.0,
    protein: 9.0,
    fat: 3.2,
  },
  "pane integrale": {
    kcal: 247,
    carbs: 41.0,
    protein: 13.0,
    fat: 4.2,
  },
  "pane ai cereali": {
    kcal: 260,
    carbs: 43.0,
    protein: 12.0,
    fat: 4.5,
  },
  "pane di segale": {
    kcal: 259,
    carbs: 48.0,
    protein: 8.5,
    fat: 3.3,
  },
  baguette: {
    kcal: 270,
    carbs: 56.0,
    protein: 8.0,
    fat: 0.5,
  },
  ciabatta: {
    kcal: 270,
    carbs: 55.0,
    protein: 8.5,
    fat: 1.5,
  },
  focaccia: {
    kcal: 275,
    carbs: 45.0,
    protein: 8.0,
    fat: 8.0,
  },
  "focaccia al rosmarino": {
    kcal: 285,
    carbs: 45.0,
    protein: 8.0,
    fat: 10.0,
  },

  "pizza margherita": {
    kcal: 270,
    carbs: 33.0,
    protein: 12.0,
    fat: 10.0,
  },
  "pizza marinara": {
    kcal: 240,
    carbs: 34.0,
    protein: 8.0,
    fat: 6.0,
  },
  "pizza quattro formaggi": {
    kcal: 310,
    carbs: 29.0,
    protein: 14.0,
    fat: 16.0,
  },
  "pizza prosciutto e funghi": {
    kcal: 285,
    carbs: 30.0,
    protein: 13.0,
    fat: 12.0,
  },
  "pizza diavola": {
    kcal: 295,
    carbs: 30.0,
    protein: 13.0,
    fat: 13.0,
  },
  "pizza bianca": {
    kcal: 265,
    carbs: 36.0,
    protein: 8.0,
    fat: 8.0,
  },
  "pizza al taglio": {
    kcal: 280,
    carbs: 35.0,
    protein: 10.0,
    fat: 10.0,
  },

  grissini: {
    kcal: 420,
    carbs: 72.0,
    protein: 12.0,
    fat: 10.0,
  },
  taralli: {
    kcal: 480,
    carbs: 63.0,
    protein: 8.0,
    fat: 20.0,
  },
  "crackers salati": {
    kcal: 421,
    carbs: 72.0,
    protein: 10.0,
    fat: 9.0,
  },
  "crackers integrali": {
    kcal: 405,
    carbs: 67.0,
    protein: 10.0,
    fat: 9.0,
  },

  piadina: {
    kcal: 310,
    carbs: 50.0,
    protein: 7.0,
    fat: 9.0,
  },
  "piadina integrale": {
    kcal: 290,
    carbs: 45.0,
    protein: 8.0,
    fat: 8.0,
  },

  "tortilla di grano": {
    kcal: 310,
    carbs: 52.0,
    protein: 8.0,
    fat: 7.0,
  },
  "tortilla di mais": {
    kcal: 218,
    carbs: 45.0,
    protein: 5.0,
    fat: 2.8,
  },

  "pan bauletto": {
    kcal: 260,
    carbs: 48.0,
    protein: 8.0,
    fat: 3.5,
  },
  "pan brioche": {
    kcal: 330,
    carbs: 50.0,
    protein: 8.0,
    fat: 10.0,
  },

  "brioche vuota": {
    kcal: 420,
    carbs: 45.0,
    protein: 7.0,
    fat: 22.0,
  },
  "cornetto vuoto": {
    kcal: 410,
    carbs: 46.0,
    protein: 7.0,
    fat: 20.0,
  },
  "cornetto alla crema": {
    kcal: 450,
    carbs: 48.0,
    protein: 7.0,
    fat: 23.0,
  },
  "cornetto al cioccolato": {
    kcal: 460,
    carbs: 47.0,
    protein: 7.0,
    fat: 25.0,
  },
  "croissant francese": {
    kcal: 406,
    carbs: 45.0,
    protein: 8.0,
    fat: 21.0,
  },

  "pane al latte": {
    kcal: 315,
    carbs: 52.0,
    protein: 9.0,
    fat: 6.0,
  },
  "panini da hamburger": {
    kcal: 280,
    carbs: 49.0,
    protein: 9.0,
    fat: 4.0,
  },
  "panini hot dog": {
    kcal: 290,
    carbs: 51.0,
    protein: 9.0,
    fat: 4.5,
  },

  crêpes: {
    kcal: 227,
    carbs: 25.0,
    protein: 7.0,
    fat: 10.0,
  },
  waffle: {
    kcal: 291,
    carbs: 32.0,
    protein: 6.0,
    fat: 15.0,
  },
};

export const pasta_riso_farine = {
  "pasta secca": {
    kcal: 353,
    carbs: 72.0,
    protein: 12.0,
    fat: 1.5,
  },
  "pasta integrale": {
    kcal: 348,
    carbs: 65.0,
    protein: 14.0,
    fat: 2.5,
  },
  "pasta all'uovo": {
    kcal: 370,
    carbs: 68.0,
    protein: 14.0,
    fat: 4.0,
  },

  spaghetti: {
    kcal: 352,
    carbs: 71.0,
    protein: 12.0,
    fat: 1.5,
  },
  penne: {
    kcal: 353,
    carbs: 72.0,
    protein: 12.0,
    fat: 1.5,
  },
  fusilli: {
    kcal: 353,
    carbs: 72.0,
    protein: 12.0,
    fat: 1.5,
  },
  rigatoni: {
    kcal: 353,
    carbs: 72.0,
    protein: 12.0,
    fat: 1.5,
  },
  farfalle: {
    kcal: 353,
    carbs: 72.0,
    protein: 12.0,
    fat: 1.5,
  },
  tagliatelle: {
    kcal: 370,
    carbs: 68.0,
    protein: 14.0,
    fat: 4.0,
  },
  "lasagne secche": {
    kcal: 360,
    carbs: 72.0,
    protein: 13.0,
    fat: 2.0,
  },

  "gnocchi di patate": {
    kcal: 160,
    carbs: 33.0,
    protein: 3.0,
    fat: 0.3,
  },
  "gnocchi alla romana": {
    kcal: 176,
    carbs: 27.0,
    protein: 6.0,
    fat: 4.0,
  },

  "riso basmati": {
    kcal: 365,
    carbs: 78.0,
    protein: 8.0,
    fat: 0.6,
  },
  "riso arborio": {
    kcal: 358,
    carbs: 79.0,
    protein: 7.0,
    fat: 0.5,
  },
  "riso carnaroli": {
    kcal: 360,
    carbs: 79.0,
    protein: 7.0,
    fat: 0.5,
  },
  "riso jasmine": {
    kcal: 360,
    carbs: 80.0,
    protein: 7.0,
    fat: 0.5,
  },
  "riso integrale": {
    kcal: 370,
    carbs: 76.0,
    protein: 8.0,
    fat: 2.7,
  },
  "riso venere": {
    kcal: 350,
    carbs: 72.0,
    protein: 8.0,
    fat: 2.5,
  },
  "riso rosso": {
    kcal: 360,
    carbs: 76.0,
    protein: 7.0,
    fat: 2.0,
  },

  "farina 00": {
    kcal: 364,
    carbs: 76.0,
    protein: 10.0,
    fat: 1.0,
  },
  "farina 0": {
    kcal: 360,
    carbs: 75.0,
    protein: 11.0,
    fat: 1.2,
  },
  "farina 1": {
    kcal: 340,
    carbs: 73.0,
    protein: 12.0,
    fat: 1.5,
  },
  "farina 2": {
    kcal: 330,
    carbs: 70.0,
    protein: 12.0,
    fat: 1.8,
  },
  "farina integrale": {
    kcal: 340,
    carbs: 68.0,
    protein: 13.0,
    fat: 2.5,
  },

  "semola rimacinata": {
    kcal: 360,
    carbs: 73.0,
    protein: 12.0,
    fat: 1.5,
  },
  semolino: {
    kcal: 360,
    carbs: 73.0,
    protein: 11.0,
    fat: 1.5,
  },

  "farina di ceci": {
    kcal: 387,
    carbs: 57.0,
    protein: 22.0,
    fat: 6.7,
  },
  "farina di castagne": {
    kcal: 369,
    carbs: 74.0,
    protein: 6.0,
    fat: 3.0,
  },
  "farina di mandorle": {
    kcal: 590,
    carbs: 19.0,
    protein: 21.0,
    fat: 50.0,
  },
  "farina di cocco": {
    kcal: 660,
    carbs: 24.0,
    protein: 7.0,
    fat: 64.0,
  },
  "farina di riso": {
    kcal: 366,
    carbs: 80.0,
    protein: 6.0,
    fat: 0.7,
  },
  "farina di mais": {
    kcal: 365,
    carbs: 76.0,
    protein: 7.0,
    fat: 3.5,
  },

  polenta: {
    kcal: 358,
    carbs: 79.0,
    protein: 8.0,
    fat: 1.0,
  },

  "amido di mais": {
    kcal: 381,
    carbs: 91.0,
    protein: 0.3,
    fat: 0.1,
  },
  "fecola di patate": {
    kcal: 351,
    carbs: 83.0,
    protein: 0.1,
    fat: 0.1,
  },
};

export const carni_salumi = {
  "petto di pollo": {
    kcal: 165,
    carbs: 0.0,
    protein: 31.0,
    fat: 3.6,
  },
  "coscia di pollo": {
    kcal: 215,
    carbs: 0.0,
    protein: 26.0,
    fat: 11.0,
  },
  "pollo arrosto": {
    kcal: 190,
    carbs: 0.0,
    protein: 28.0,
    fat: 8.0,
  },

  tacchino: {
    kcal: 135,
    carbs: 0.0,
    protein: 29.0,
    fat: 1.0,
  },
  "fesa di tacchino": {
    kcal: 104,
    carbs: 1.0,
    protein: 23.0,
    fat: 1.0,
  },

  "manzo magro": {
    kcal: 250,
    carbs: 0.0,
    protein: 26.0,
    fat: 15.0,
  },
  "manzo grasso": {
    kcal: 310,
    carbs: 0.0,
    protein: 24.0,
    fat: 22.0,
  },
  "bistecca di manzo": {
    kcal: 271,
    carbs: 0.0,
    protein: 25.0,
    fat: 18.0,
  },

  vitello: {
    kcal: 195,
    carbs: 0.0,
    protein: 30.0,
    fat: 7.0,
  },

  "maiale magro": {
    kcal: 242,
    carbs: 0.0,
    protein: 27.0,
    fat: 14.0,
  },
  "maiale grasso": {
    kcal: 320,
    carbs: 0.0,
    protein: 20.0,
    fat: 27.0,
  },
  "lonza di maiale": {
    kcal: 210,
    carbs: 0.0,
    protein: 29.0,
    fat: 9.0,
  },
  "costine di maiale": {
    kcal: 320,
    carbs: 0.0,
    protein: 20.0,
    fat: 28.0,
  },

  salsiccia: {
    kcal: 330,
    carbs: 1.0,
    protein: 16.0,
    fat: 29.0,
  },
  wurstel: {
    kcal: 270,
    carbs: 2.0,
    protein: 12.0,
    fat: 23.0,
  },
  "hamburger di manzo": {
    kcal: 295,
    carbs: 0.0,
    protein: 26.0,
    fat: 20.0,
  },

  agnello: {
    kcal: 294,
    carbs: 0.0,
    protein: 25.0,
    fat: 21.0,
  },
  capretto: {
    kcal: 240,
    carbs: 0.0,
    protein: 27.0,
    fat: 14.0,
  },
  coniglio: {
    kcal: 170,
    carbs: 0.0,
    protein: 33.0,
    fat: 3.5,
  },

  anatra: {
    kcal: 337,
    carbs: 0.0,
    protein: 19.0,
    fat: 28.0,
  },
  oca: {
    kcal: 371,
    carbs: 0.0,
    protein: 16.0,
    fat: 33.0,
  },

  "prosciutto cotto": {
    kcal: 145,
    carbs: 1.0,
    protein: 20.0,
    fat: 6.0,
  },
  "prosciutto crudo": {
    kcal: 250,
    carbs: 0.0,
    protein: 28.0,
    fat: 14.0,
  },
  speck: {
    kcal: 300,
    carbs: 0.0,
    protein: 30.0,
    fat: 20.0,
  },
  bresaola: {
    kcal: 151,
    carbs: 2.0,
    protein: 32.0,
    fat: 2.0,
  },

  salame: {
    kcal: 410,
    carbs: 1.0,
    protein: 22.0,
    fat: 36.0,
  },
  "salame milano": {
    kcal: 430,
    carbs: 1.0,
    protein: 21.0,
    fat: 38.0,
  },
  "salame ungherese": {
    kcal: 450,
    carbs: 1.0,
    protein: 20.0,
    fat: 41.0,
  },

  mortadella: {
    kcal: 311,
    carbs: 1.0,
    protein: 16.0,
    fat: 27.0,
  },
  pancetta: {
    kcal: 458,
    carbs: 0.0,
    protein: 13.0,
    fat: 45.0,
  },
  guanciale: {
    kcal: 655,
    carbs: 0.0,
    protein: 8.0,
    fat: 70.0,
  },
  lardo: {
    kcal: 795,
    carbs: 0.0,
    protein: 1.0,
    fat: 88.0,
  },

  coppa: {
    kcal: 340,
    carbs: 0.0,
    protein: 22.0,
    fat: 27.0,
  },
  soppressata: {
    kcal: 420,
    carbs: 1.0,
    protein: 22.0,
    fat: 37.0,
  },
  porchetta: {
    kcal: 320,
    carbs: 0.0,
    protein: 22.0,
    fat: 25.0,
  },

  zampone: {
    kcal: 270,
    carbs: 1.0,
    protein: 16.0,
    fat: 22.0,
  },
  cotechino: {
    kcal: 310,
    carbs: 1.0,
    protein: 14.0,
    fat: 28.0,
  },

  trippa: {
    kcal: 96,
    carbs: 0.0,
    protein: 12.0,
    fat: 4.0,
  },
  "fegato di manzo": {
    kcal: 135,
    carbs: 4.0,
    protein: 20.0,
    fat: 3.5,
  },
  "fegato di pollo": {
    kcal: 119,
    carbs: 1.0,
    protein: 17.0,
    fat: 4.8,
  },
};

export const pesce_frutti_di_mare = {
  salmone: {
    kcal: 208,
    carbs: 0.0,
    protein: 20.0,
    fat: 13.0,
  },
  "salmone affumicato": {
    kcal: 117,
    carbs: 0.0,
    protein: 18.0,
    fat: 4.0,
  },

  "tonno fresco": {
    kcal: 132,
    carbs: 0.0,
    protein: 29.0,
    fat: 1.0,
  },
  "tonno in scatola al naturale": {
    kcal: 116,
    carbs: 0.0,
    protein: 25.0,
    fat: 1.0,
  },
  "tonno in scatola all'olio": {
    kcal: 198,
    carbs: 0.0,
    protein: 25.0,
    fat: 8.0,
  },

  sgombro: {
    kcal: 205,
    carbs: 0.0,
    protein: 19.0,
    fat: 14.0,
  },
  "sgombro in scatola": {
    kcal: 190,
    carbs: 0.0,
    protein: 20.0,
    fat: 12.0,
  },

  merluzzo: {
    kcal: 82,
    carbs: 0.0,
    protein: 18.0,
    fat: 0.7,
  },
  nasello: {
    kcal: 90,
    carbs: 0.0,
    protein: 19.0,
    fat: 1.0,
  },

  orata: {
    kcal: 121,
    carbs: 0.0,
    protein: 20.0,
    fat: 4.0,
  },
  branzino: {
    kcal: 124,
    carbs: 0.0,
    protein: 21.0,
    fat: 4.0,
  },
  spigola: {
    kcal: 124,
    carbs: 0.0,
    protein: 21.0,
    fat: 4.0,
  },

  trota: {
    kcal: 148,
    carbs: 0.0,
    protein: 20.0,
    fat: 7.0,
  },
  "pesce spada": {
    kcal: 144,
    carbs: 0.0,
    protein: 23.0,
    fat: 5.0,
  },

  alici: {
    kcal: 210,
    carbs: 0.0,
    protein: 29.0,
    fat: 10.0,
  },
  "acciughe sott'olio": {
    kcal: 210,
    carbs: 0.0,
    protein: 29.0,
    fat: 10.0,
  },

  sardine: {
    kcal: 208,
    carbs: 0.0,
    protein: 25.0,
    fat: 11.0,
  },
  "sardine in scatola": {
    kcal: 185,
    carbs: 0.0,
    protein: 24.0,
    fat: 10.0,
  },

  gamberi: {
    kcal: 99,
    carbs: 0.2,
    protein: 24.0,
    fat: 0.3,
  },
  gamberetti: {
    kcal: 85,
    carbs: 0.2,
    protein: 20.0,
    fat: 0.3,
  },
  scampi: {
    kcal: 90,
    carbs: 0.0,
    protein: 19.0,
    fat: 1.0,
  },

  aragosta: {
    kcal: 89,
    carbs: 0.0,
    protein: 19.0,
    fat: 1.0,
  },
  astice: {
    kcal: 98,
    carbs: 1.0,
    protein: 21.0,
    fat: 1.0,
  },

  polpo: {
    kcal: 82,
    carbs: 2.2,
    protein: 15.0,
    fat: 1.0,
  },
  calamari: {
    kcal: 92,
    carbs: 3.1,
    protein: 16.0,
    fat: 1.4,
  },
  seppie: {
    kcal: 79,
    carbs: 1.0,
    protein: 16.0,
    fat: 1.0,
  },

  cozze: {
    kcal: 86,
    carbs: 3.7,
    protein: 12.0,
    fat: 2.2,
  },
  vongole: {
    kcal: 74,
    carbs: 2.6,
    protein: 12.0,
    fat: 0.8,
  },
  ostriche: {
    kcal: 81,
    carbs: 4.9,
    protein: 9.0,
    fat: 2.0,
  },
  capasanta: {
    kcal: 69,
    carbs: 3.2,
    protein: 12.0,
    fat: 0.8,
  },

  granchio: {
    kcal: 97,
    carbs: 0.0,
    protein: 21.0,
    fat: 1.5,
  },
  "granchio in scatola": {
    kcal: 100,
    carbs: 0.0,
    protein: 18.0,
    fat: 2.0,
  },

  "alga nori": {
    kcal: 35,
    carbs: 5.0,
    protein: 5.8,
    fat: 0.3,
  },

  surimi: {
    kcal: 95,
    carbs: 12.0,
    protein: 7.0,
    fat: 1.0,
  },

  baccalà: {
    kcal: 105,
    carbs: 0.0,
    protein: 23.0,
    fat: 1.0,
  },
};

export const latticini_formaggi = {
  "latte intero": {
    kcal: 64,
    carbs: 5.0,
    protein: 3.3,
    fat: 3.6,
  },
  "latte parzialmente scremato": {
    kcal: 46,
    carbs: 4.9,
    protein: 3.4,
    fat: 1.6,
  },
  "latte scremato": {
    kcal: 35,
    carbs: 5.0,
    protein: 3.4,
    fat: 0.1,
  },
  "latte di capra": {
    kcal: 69,
    carbs: 4.5,
    protein: 3.6,
    fat: 4.1,
  },
  "latte di pecora": {
    kcal: 102,
    carbs: 5.2,
    protein: 5.5,
    fat: 6.5,
  },

  "yogurt intero": {
    kcal: 61,
    carbs: 4.7,
    protein: 3.5,
    fat: 3.3,
  },
  "yogurt magro": {
    kcal: 36,
    carbs: 5.0,
    protein: 4.0,
    fat: 0.2,
  },
  "yogurt greco": {
    kcal: 115,
    carbs: 3.8,
    protein: 10.0,
    fat: 8.0,
  },

  "panna fresca": {
    kcal: 340,
    carbs: 3.0,
    protein: 2.0,
    fat: 35.0,
  },
  "panna da cucina": {
    kcal: 195,
    carbs: 3.0,
    protein: 3.0,
    fat: 19.0,
  },
  "panna montata": {
    kcal: 257,
    carbs: 12.0,
    protein: 2.0,
    fat: 22.0,
  },

  burro: {
    kcal: 717,
    carbs: 0.1,
    protein: 0.5,
    fat: 81.0,
  },
  "burro chiarificato": {
    kcal: 900,
    carbs: 0.0,
    protein: 0.0,
    fat: 100.0,
  },
  margarina: {
    kcal: 717,
    carbs: 0.7,
    protein: 0.2,
    fat: 80.0,
  },

  "ricotta vaccina": {
    kcal: 146,
    carbs: 3.0,
    protein: 8.0,
    fat: 10.0,
  },
  "ricotta di pecora": {
    kcal: 157,
    carbs: 3.0,
    protein: 9.0,
    fat: 11.0,
  },
  "ricotta di capra": {
    kcal: 174,
    carbs: 3.0,
    protein: 10.0,
    fat: 13.0,
  },

  mozzarella: {
    kcal: 280,
    carbs: 2.0,
    protein: 18.0,
    fat: 21.0,
  },
  "mozzarella di bufala": {
    kcal: 288,
    carbs: 2.0,
    protein: 16.0,
    fat: 24.0,
  },
  fiordilatte: {
    kcal: 250,
    carbs: 2.0,
    protein: 17.0,
    fat: 19.0,
  },

  scamorza: {
    kcal: 330,
    carbs: 1.5,
    protein: 25.0,
    fat: 25.0,
  },
  "scamorza affumicata": {
    kcal: 340,
    carbs: 1.5,
    protein: 25.0,
    fat: 26.0,
  },
  provola: {
    kcal: 330,
    carbs: 2.0,
    protein: 26.0,
    fat: 25.0,
  },
  provolone: {
    kcal: 360,
    carbs: 2.0,
    protein: 26.0,
    fat: 28.0,
  },

  caciocavallo: {
    kcal: 380,
    carbs: 2.0,
    protein: 25.0,
    fat: 30.0,
  },
  crescenza: {
    kcal: 280,
    carbs: 2.0,
    protein: 17.0,
    fat: 22.0,
  },
  stracchino: {
    kcal: 290,
    carbs: 2.0,
    protein: 17.0,
    fat: 23.0,
  },
  robiola: {
    kcal: 333,
    carbs: 2.0,
    protein: 11.0,
    fat: 30.0,
  },

  "gorgonzola dolce": {
    kcal: 330,
    carbs: 2.0,
    protein: 19.0,
    fat: 27.0,
  },
  "gorgonzola piccante": {
    kcal: 360,
    carbs: 2.0,
    protein: 21.0,
    fat: 29.0,
  },

  "parmigiano reggiano": {
    kcal: 402,
    carbs: 0.0,
    protein: 33.0,
    fat: 29.0,
  },
  "grana padano": {
    kcal: 398,
    carbs: 0.0,
    protein: 33.0,
    fat: 28.0,
  },

  "pecorino romano": {
    kcal: 387,
    carbs: 0.0,
    protein: 28.0,
    fat: 32.0,
  },
  "pecorino toscano": {
    kcal: 380,
    carbs: 0.0,
    protein: 25.0,
    fat: 31.0,
  },

  emmental: {
    kcal: 380,
    carbs: 2.0,
    protein: 28.0,
    fat: 29.0,
  },
  gruviera: {
    kcal: 413,
    carbs: 0.4,
    protein: 30.0,
    fat: 34.0,
  },
  fontina: {
    kcal: 389,
    carbs: 1.0,
    protein: 26.0,
    fat: 31.0,
  },
  asiago: {
    kcal: 330,
    carbs: 1.0,
    protein: 25.0,
    fat: 25.0,
  },
  edamer: {
    kcal: 357,
    carbs: 1.4,
    protein: 25.0,
    fat: 27.0,
  },

  brie: {
    kcal: 334,
    carbs: 0.5,
    protein: 21.0,
    fat: 28.0,
  },
  camembert: {
    kcal: 300,
    carbs: 0.5,
    protein: 19.0,
    fat: 24.0,
  },

  philadelphia: {
    kcal: 253,
    carbs: 4.0,
    protein: 6.0,
    fat: 24.0,
  },
  mascarpone: {
    kcal: 460,
    carbs: 3.0,
    protein: 4.0,
    fat: 47.0,
  },

  kefir: {
    kcal: 55,
    carbs: 4.0,
    protein: 3.5,
    fat: 3.0,
  },
};

export const verdure = {
  pomodoro: {
    kcal: 18,
    carbs: 3.9,
    protein: 0.9,
    fat: 0.2,
  },
  pomodorini: {
    kcal: 20,
    carbs: 4.0,
    protein: 1.0,
    fat: 0.3,
  },

  zucchine: {
    kcal: 17,
    carbs: 3.1,
    protein: 1.2,
    fat: 0.3,
  },
  melanzane: {
    kcal: 25,
    carbs: 5.9,
    protein: 1.0,
    fat: 0.2,
  },

  "peperoni rossi": {
    kcal: 31,
    carbs: 6.0,
    protein: 1.0,
    fat: 0.3,
  },
  "peperoni gialli": {
    kcal: 27,
    carbs: 5.5,
    protein: 1.0,
    fat: 0.2,
  },
  "peperoni verdi": {
    kcal: 20,
    carbs: 4.6,
    protein: 0.9,
    fat: 0.2,
  },

  carote: {
    kcal: 41,
    carbs: 10.0,
    protein: 0.9,
    fat: 0.2,
  },

  cipolla: {
    kcal: 40,
    carbs: 9.3,
    protein: 1.1,
    fat: 0.1,
  },
  "cipolla rossa": {
    kcal: 38,
    carbs: 9.0,
    protein: 1.0,
    fat: 0.1,
  },
  cipollotto: {
    kcal: 32,
    carbs: 7.3,
    protein: 1.8,
    fat: 0.2,
  },

  porro: {
    kcal: 61,
    carbs: 14.0,
    protein: 1.5,
    fat: 0.3,
  },
  aglio: {
    kcal: 149,
    carbs: 33.0,
    protein: 6.4,
    fat: 0.5,
  },

  patate: {
    kcal: 77,
    carbs: 17.0,
    protein: 2.0,
    fat: 0.1,
  },
  "patate dolci": {
    kcal: 86,
    carbs: 20.0,
    protein: 1.6,
    fat: 0.1,
  },

  broccoli: {
    kcal: 34,
    carbs: 7.0,
    protein: 2.8,
    fat: 0.4,
  },
  cavolfiore: {
    kcal: 25,
    carbs: 5.0,
    protein: 2.0,
    fat: 0.3,
  },
  "cavolo cappuccio": {
    kcal: 25,
    carbs: 6.0,
    protein: 1.3,
    fat: 0.1,
  },
  "cavolo rosso": {
    kcal: 31,
    carbs: 7.4,
    protein: 1.4,
    fat: 0.2,
  },
  "cavolo nero": {
    kcal: 49,
    carbs: 9.0,
    protein: 4.3,
    fat: 0.9,
  },
  verza: {
    kcal: 27,
    carbs: 6.0,
    protein: 1.5,
    fat: 0.2,
  },

  spinaci: {
    kcal: 23,
    carbs: 3.6,
    protein: 2.9,
    fat: 0.4,
  },
  bietole: {
    kcal: 19,
    carbs: 3.7,
    protein: 1.8,
    fat: 0.2,
  },

  lattuga: {
    kcal: 15,
    carbs: 2.9,
    protein: 1.4,
    fat: 0.2,
  },
  radicchio: {
    kcal: 23,
    carbs: 4.5,
    protein: 1.4,
    fat: 0.3,
  },
  rucola: {
    kcal: 25,
    carbs: 3.7,
    protein: 2.6,
    fat: 0.7,
  },
  indivia: {
    kcal: 17,
    carbs: 3.4,
    protein: 1.3,
    fat: 0.2,
  },

  finocchi: {
    kcal: 31,
    carbs: 7.3,
    protein: 1.2,
    fat: 0.2,
  },
  sedano: {
    kcal: 16,
    carbs: 3.0,
    protein: 0.7,
    fat: 0.2,
  },

  asparagi: {
    kcal: 20,
    carbs: 3.9,
    protein: 2.2,
    fat: 0.1,
  },
  carciofi: {
    kcal: 47,
    carbs: 11.0,
    protein: 3.3,
    fat: 0.2,
  },

  "funghi champignon": {
    kcal: 22,
    carbs: 3.3,
    protein: 3.1,
    fat: 0.3,
  },
  "funghi porcini": {
    kcal: 26,
    carbs: 3.4,
    protein: 3.7,
    fat: 0.4,
  },

  zucca: {
    kcal: 26,
    carbs: 6.5,
    protein: 1.0,
    fat: 0.1,
  },

  fagiolini: {
    kcal: 31,
    carbs: 7.0,
    protein: 1.8,
    fat: 0.1,
  },
  "piselli freschi": {
    kcal: 81,
    carbs: 14.0,
    protein: 5.4,
    fat: 0.4,
  },

  "mais dolce": {
    kcal: 86,
    carbs: 19.0,
    protein: 3.2,
    fat: 1.2,
  },

  barbabietola: {
    kcal: 43,
    carbs: 10.0,
    protein: 1.6,
    fat: 0.2,
  },
  rapa: {
    kcal: 28,
    carbs: 6.4,
    protein: 0.9,
    fat: 0.1,
  },

  "germogli di soia": {
    kcal: 49,
    carbs: 4.0,
    protein: 5.0,
    fat: 2.0,
  },
};

export const frutta_fresca = {
  mela: {
    kcal: 52,
    carbs: 14.0,
    protein: 0.3,
    fat: 0.2,
  },
  pera: {
    kcal: 57,
    carbs: 15.0,
    protein: 0.4,
    fat: 0.1,
  },
  banana: {
    kcal: 89,
    carbs: 23.0,
    protein: 1.1,
    fat: 0.3,
  },
  arancia: {
    kcal: 47,
    carbs: 12.0,
    protein: 0.9,
    fat: 0.1,
  },
  mandarino: {
    kcal: 53,
    carbs: 13.0,
    protein: 0.8,
    fat: 0.3,
  },
  clementina: {
    kcal: 47,
    carbs: 12.0,
    protein: 0.9,
    fat: 0.2,
  },
  limone: {
    kcal: 29,
    carbs: 9.3,
    protein: 1.1,
    fat: 0.3,
  },
  lime: {
    kcal: 30,
    carbs: 11.0,
    protein: 0.7,
    fat: 0.2,
  },
  pompelmo: {
    kcal: 42,
    carbs: 11.0,
    protein: 0.8,
    fat: 0.1,
  },

  albicocca: {
    kcal: 48,
    carbs: 11.0,
    protein: 1.4,
    fat: 0.4,
  },
  pesca: {
    kcal: 39,
    carbs: 10.0,
    protein: 0.9,
    fat: 0.3,
  },
  "pesca noce": {
    kcal: 44,
    carbs: 11.0,
    protein: 1.1,
    fat: 0.3,
  },
  nettaria: {
    kcal: 44,
    carbs: 11.0,
    protein: 1.1,
    fat: 0.3,
  },
  susina: {
    kcal: 46,
    carbs: 11.0,
    protein: 0.7,
    fat: 0.3,
  },

  ciliegie: {
    kcal: 63,
    carbs: 16.0,
    protein: 1.1,
    fat: 0.2,
  },
  fragole: {
    kcal: 32,
    carbs: 7.7,
    protein: 0.7,
    fat: 0.3,
  },
  lamponi: {
    kcal: 52,
    carbs: 12.0,
    protein: 1.2,
    fat: 0.7,
  },
  mirtilli: {
    kcal: 57,
    carbs: 14.0,
    protein: 0.7,
    fat: 0.3,
  },
  more: {
    kcal: 43,
    carbs: 10.0,
    protein: 1.4,
    fat: 0.5,
  },

  "uva bianca": {
    kcal: 69,
    carbs: 18.0,
    protein: 0.7,
    fat: 0.2,
  },
  "uva nera": {
    kcal: 72,
    carbs: 18.0,
    protein: 0.7,
    fat: 0.2,
  },

  kiwi: {
    kcal: 61,
    carbs: 15.0,
    protein: 1.1,
    fat: 0.5,
  },
  ananas: {
    kcal: 50,
    carbs: 13.0,
    protein: 0.5,
    fat: 0.1,
  },
  mango: {
    kcal: 60,
    carbs: 15.0,
    protein: 0.8,
    fat: 0.4,
  },
  papaya: {
    kcal: 43,
    carbs: 11.0,
    protein: 0.5,
    fat: 0.3,
  },

  melone: {
    kcal: 34,
    carbs: 8.0,
    protein: 0.8,
    fat: 0.2,
  },
  anguria: {
    kcal: 30,
    carbs: 8.0,
    protein: 0.6,
    fat: 0.2,
  },

  fico: {
    kcal: 74,
    carbs: 19.0,
    protein: 0.8,
    fat: 0.3,
  },
  "fico d'india": {
    kcal: 41,
    carbs: 10.0,
    protein: 0.7,
    fat: 0.5,
  },
  cachi: {
    kcal: 70,
    carbs: 18.0,
    protein: 0.6,
    fat: 0.2,
  },

  avocado: {
    kcal: 160,
    carbs: 9.0,
    protein: 2.0,
    fat: 15.0,
  },
  melagrana: {
    kcal: 83,
    carbs: 19.0,
    protein: 1.7,
    fat: 1.2,
  },

  "pera abate": {
    kcal: 58,
    carbs: 15.0,
    protein: 0.4,
    fat: 0.1,
  },
  "pera kaiser": {
    kcal: 57,
    carbs: 15.0,
    protein: 0.4,
    fat: 0.1,
  },
  "pera williams": {
    kcal: 61,
    carbs: 16.0,
    protein: 0.4,
    fat: 0.1,
  },

  "mela golden": {
    kcal: 52,
    carbs: 14.0,
    protein: 0.3,
    fat: 0.2,
  },
  "mela fuji": {
    kcal: 57,
    carbs: 15.0,
    protein: 0.3,
    fat: 0.2,
  },
  "mela granny smith": {
    kcal: 52,
    carbs: 14.0,
    protein: 0.3,
    fat: 0.2,
  },

  prugna: {
    kcal: 46,
    carbs: 11.0,
    protein: 0.7,
    fat: 0.3,
  },
  litchi: {
    kcal: 66,
    carbs: 17.0,
    protein: 0.8,
    fat: 0.4,
  },
};

export const dolci_creme = {
  zucchero: {
    kcal: 400,
    carbs: 100.0,
    protein: 0.0,
    fat: 0.0,
  },
  miele: {
    kcal: 304,
    carbs: 82.0,
    protein: 0.3,
    fat: 0.0,
  },
  "sciroppo d'acero": {
    kcal: 260,
    carbs: 67.0,
    protein: 0.0,
    fat: 0.0,
  },

  "cioccolato fondente": {
    kcal: 546,
    carbs: 46.0,
    protein: 4.9,
    fat: 31.0,
  },
  "cioccolato al latte": {
    kcal: 535,
    carbs: 59.0,
    protein: 7.0,
    fat: 30.0,
  },
  "cioccolato bianco": {
    kcal: 539,
    carbs: 59.0,
    protein: 6.0,
    fat: 32.0,
  },

  nutella: {
    kcal: 539,
    carbs: 57.0,
    protein: 6.0,
    fat: 31.0,
  },
  "crema spalmabile al pistacchio": {
    kcal: 580,
    carbs: 50.0,
    protein: 8.0,
    fat: 38.0,
  },

  "crema pasticcera": {
    kcal: 215,
    carbs: 30.0,
    protein: 4.0,
    fat: 8.0,
  },
  "crema chantilly": {
    kcal: 280,
    carbs: 20.0,
    protein: 2.0,
    fat: 22.0,
  },
  "crema al mascarpone": {
    kcal: 430,
    carbs: 12.0,
    protein: 4.0,
    fat: 40.0,
  },

  "panna montata": {
    kcal: 257,
    carbs: 12.0,
    protein: 2.0,
    fat: 22.0,
  },

  marmellata: {
    kcal: 250,
    carbs: 65.0,
    protein: 0.4,
    fat: 0.2,
  },
  confettura: {
    kcal: 240,
    carbs: 60.0,
    protein: 0.5,
    fat: 0.2,
  },
  "gelatina di frutta": {
    kcal: 150,
    carbs: 38.0,
    protein: 0.0,
    fat: 0.0,
  },

  caramello: {
    kcal: 382,
    carbs: 97.0,
    protein: 0.0,
    fat: 0.0,
  },
  "dulce de leche": {
    kcal: 315,
    carbs: 55.0,
    protein: 7.0,
    fat: 6.0,
  },

  meringa: {
    kcal: 384,
    carbs: 96.0,
    protein: 3.0,
    fat: 0.0,
  },

  "biscotti secchi": {
    kcal: 430,
    carbs: 75.0,
    protein: 7.0,
    fat: 10.0,
  },
  "biscotti al burro": {
    kcal: 510,
    carbs: 60.0,
    protein: 6.0,
    fat: 26.0,
  },
  savoiardi: {
    kcal: 389,
    carbs: 84.0,
    protein: 8.0,
    fat: 4.0,
  },
  pavesini: {
    kcal: 360,
    carbs: 80.0,
    protein: 6.0,
    fat: 2.0,
  },

  "pan di spagna": {
    kcal: 297,
    carbs: 52.0,
    protein: 6.0,
    fat: 6.0,
  },
  brownie: {
    kcal: 466,
    carbs: 60.0,
    protein: 5.0,
    fat: 22.0,
  },
  "torta al cioccolato": {
    kcal: 389,
    carbs: 55.0,
    protein: 5.0,
    fat: 16.0,
  },

  "croissant ripieno": {
    kcal: 460,
    carbs: 47.0,
    protein: 7.0,
    fat: 25.0,
  },
  ciambella: {
    kcal: 420,
    carbs: 55.0,
    protein: 6.0,
    fat: 18.0,
  },

  "gelato alla vaniglia": {
    kcal: 207,
    carbs: 24.0,
    protein: 3.5,
    fat: 10.0,
  },
  "gelato al cioccolato": {
    kcal: 216,
    carbs: 28.0,
    protein: 4.0,
    fat: 10.0,
  },
  "sorbetto al limone": {
    kcal: 130,
    carbs: 32.0,
    protein: 0.0,
    fat: 0.0,
  },
};

export const oli_grassi = {
  "olio extravergine di oliva": {
    kcal: 884,
    carbs: 0.0,
    protein: 0.0,
    fat: 100.0,
  },
  "olio di oliva": {
    kcal: 884,
    carbs: 0.0,
    protein: 0.0,
    fat: 100.0,
  },
  "olio di semi di girasole": {
    kcal: 884,
    carbs: 0.0,
    protein: 0.0,
    fat: 100.0,
  },
  "olio di semi di mais": {
    kcal: 884,
    carbs: 0.0,
    protein: 0.0,
    fat: 100.0,
  },
  "olio di arachidi": {
    kcal: 884,
    carbs: 0.0,
    protein: 0.0,
    fat: 100.0,
  },
  "olio di soia": {
    kcal: 884,
    carbs: 0.0,
    protein: 0.0,
    fat: 100.0,
  },
  "olio di colza": {
    kcal: 884,
    carbs: 0.0,
    protein: 0.0,
    fat: 100.0,
  },
  "olio di cocco": {
    kcal: 892,
    carbs: 0.0,
    protein: 0.0,
    fat: 100.0,
  },
  "olio di palma": {
    kcal: 884,
    carbs: 0.0,
    protein: 0.0,
    fat: 100.0,
  },
  "olio di sesamo": {
    kcal: 884,
    carbs: 0.0,
    protein: 0.0,
    fat: 100.0,
  },

  burro: {
    kcal: 717,
    carbs: 0.1,
    protein: 0.5,
    fat: 81.0,
  },
  "burro chiarificato (ghee)": {
    kcal: 900,
    carbs: 0.0,
    protein: 0.0,
    fat: 100.0,
  },
  strutto: {
    kcal: 902,
    carbs: 0.0,
    protein: 0.0,
    fat: 100.0,
  },
  margarina: {
    kcal: 717,
    carbs: 0.7,
    protein: 0.2,
    fat: 80.0,
  },

  maionese: {
    kcal: 680,
    carbs: 1.0,
    protein: 1.0,
    fat: 75.0,
  },
};

export const salse_base = {
  ketchup: {
    kcal: 112,
    carbs: 26.0,
    protein: 1.3,
    fat: 0.1,
  },
  maionese: {
    kcal: 680,
    carbs: 1.0,
    protein: 1.0,
    fat: 75.0,
  },
  "maionese light": {
    kcal: 260,
    carbs: 6.0,
    protein: 1.0,
    fat: 25.0,
  },
  senape: {
    kcal: 66,
    carbs: 5.0,
    protein: 4.0,
    fat: 4.0,
  },
  "salsa barbecue": {
    kcal: 165,
    carbs: 40.0,
    protein: 1.0,
    fat: 0.3,
  },
  "salsa tartara": {
    kcal: 220,
    carbs: 6.0,
    protein: 1.0,
    fat: 20.0,
  },
  "salsa rosa": {
    kcal: 430,
    carbs: 6.0,
    protein: 1.0,
    fat: 42.0,
  },
  "salsa yogurt": {
    kcal: 80,
    carbs: 6.0,
    protein: 3.0,
    fat: 5.0,
  },
  "salsa tonnata": {
    kcal: 270,
    carbs: 3.0,
    protein: 8.0,
    fat: 24.0,
  },
  "salsa aioli": {
    kcal: 700,
    carbs: 1.0,
    protein: 1.0,
    fat: 78.0,
  },
  "salsa guacamole": {
    kcal: 160,
    carbs: 9.0,
    protein: 2.0,
    fat: 14.0,
  },
  "salsa hummus": {
    kcal: 166,
    carbs: 14.0,
    protein: 8.0,
    fat: 10.0,
  },
};

export const salse_italiane = {
  "sugo di pomodoro": {
    kcal: 55,
    carbs: 8.0,
    protein: 1.5,
    fat: 1.5,
  },
  "passata di pomodoro": {
    kcal: 29,
    carbs: 6.0,
    protein: 1.4,
    fat: 0.2,
  },
  "sugo al basilico": {
    kcal: 60,
    carbs: 8.0,
    protein: 1.5,
    fat: 2.0,
  },
  "sugo all'arrabbiata": {
    kcal: 70,
    carbs: 9.0,
    protein: 1.5,
    fat: 3.0,
  },
  "sugo alle olive": {
    kcal: 95,
    carbs: 7.0,
    protein: 1.5,
    fat: 6.0,
  },
  "sugo ai funghi": {
    kcal: 80,
    carbs: 8.0,
    protein: 2.0,
    fat: 4.0,
  },

  "pesto alla genovese": {
    kcal: 450,
    carbs: 4.0,
    protein: 4.0,
    fat: 45.0,
  },
  "pesto rosso": {
    kcal: 380,
    carbs: 8.0,
    protein: 5.0,
    fat: 34.0,
  },

  "ragù alla bolognese": {
    kcal: 150,
    carbs: 5.0,
    protein: 10.0,
    fat: 10.0,
  },
  "ragù di salsiccia": {
    kcal: 190,
    carbs: 4.0,
    protein: 9.0,
    fat: 15.0,
  },
  "ragù di pollo": {
    kcal: 120,
    carbs: 4.0,
    protein: 12.0,
    fat: 6.0,
  },

  besciamella: {
    kcal: 105,
    carbs: 7.0,
    protein: 3.0,
    fat: 7.0,
  },
  "besciamella light": {
    kcal: 65,
    carbs: 7.0,
    protein: 3.0,
    fat: 3.0,
  },

  "salsa carbonara": {
    kcal: 220,
    carbs: 3.0,
    protein: 10.0,
    fat: 18.0,
  },
  "salsa cacio e pepe": {
    kcal: 310,
    carbs: 3.0,
    protein: 18.0,
    fat: 24.0,
  },
  "salsa al tartufo": {
    kcal: 180,
    carbs: 4.0,
    protein: 3.0,
    fat: 16.0,
  },

  "salsa al gorgonzola": {
    kcal: 240,
    carbs: 3.0,
    protein: 6.0,
    fat: 22.0,
  },
  "salsa ai quattro formaggi": {
    kcal: 280,
    carbs: 4.0,
    protein: 8.0,
    fat: 24.0,
  },

  "salsa puttanesca": {
    kcal: 95,
    carbs: 8.0,
    protein: 2.0,
    fat: 6.0,
  },
  "salsa amatriciana": {
    kcal: 160,
    carbs: 7.0,
    protein: 6.0,
    fat: 12.0,
  },
  "salsa alla norma": {
    kcal: 110,
    carbs: 10.0,
    protein: 2.0,
    fat: 7.0,
  },

  "salsa al limone": {
    kcal: 90,
    carbs: 6.0,
    protein: 1.0,
    fat: 7.0,
  },
  "salsa al vino bianco": {
    kcal: 70,
    carbs: 4.0,
    protein: 1.0,
    fat: 5.0,
  },
  "salsa verde": {
    kcal: 220,
    carbs: 4.0,
    protein: 3.0,
    fat: 20.0,
  },
};

export const salse_asiatiche = {
  "salsa di soia": {
    kcal: 53,
    carbs: 4.9,
    protein: 8.1,
    fat: 0.0,
  },
  "salsa di soia light": {
    kcal: 40,
    carbs: 3.0,
    protein: 6.0,
    fat: 0.0,
  },
  tamari: {
    kcal: 60,
    carbs: 5.0,
    protein: 10.0,
    fat: 0.0,
  },

  "salsa teriyaki": {
    kcal: 89,
    carbs: 14.0,
    protein: 5.0,
    fat: 0.0,
  },
  "salsa hoisin": {
    kcal: 220,
    carbs: 39.0,
    protein: 3.0,
    fat: 3.0,
  },
  "salsa sweet chili": {
    kcal: 200,
    carbs: 50.0,
    protein: 0.0,
    fat: 0.0,
  },
  "salsa ponzu": {
    kcal: 30,
    carbs: 6.0,
    protein: 1.0,
    fat: 0.0,
  },

  "salsa al curry giapponese": {
    kcal: 110,
    carbs: 15.0,
    protein: 2.0,
    fat: 5.0,
  },
  "salsa al curry thai (rossa)": {
    kcal: 150,
    carbs: 7.0,
    protein: 3.0,
    fat: 12.0,
  },
  "salsa al curry thai (verde)": {
    kcal: 140,
    carbs: 6.0,
    protein: 3.0,
    fat: 11.0,
  },

  "salsa satay": {
    kcal: 280,
    carbs: 10.0,
    protein: 10.0,
    fat: 22.0,
  },
  "tahina (tahini)": {
    kcal: 595,
    carbs: 21.0,
    protein: 17.0,
    fat: 53.0,
  },

  "salsa miso": {
    kcal: 199,
    carbs: 26.0,
    protein: 12.0,
    fat: 6.0,
  },
  "salsa gochujang": {
    kcal: 190,
    carbs: 44.0,
    protein: 4.0,
    fat: 1.0,
  },
  "salsa kimchi": {
    kcal: 15,
    carbs: 2.0,
    protein: 1.1,
    fat: 0.2,
  },

  "salsa oyster (ostrica)": {
    kcal: 51,
    carbs: 11.0,
    protein: 1.0,
    fat: 0.0,
  },
  "salsa fish sauce (nam pla)": {
    kcal: 60,
    carbs: 0.0,
    protein: 10.0,
    fat: 0.0,
  },

  "salsa yakitori": {
    kcal: 120,
    carbs: 25.0,
    protein: 3.0,
    fat: 0.0,
  },
  "salsa unagi": {
    kcal: 150,
    carbs: 30.0,
    protein: 3.0,
    fat: 0.0,
  },

  "salsa al sesamo (goma dare)": {
    kcal: 320,
    carbs: 10.0,
    protein: 7.0,
    fat: 28.0,
  },
  "salsa al peperoncino (sambal oelek)": {
    kcal: 40,
    carbs: 8.0,
    protein: 1.0,
    fat: 0.5,
  },
};

export const salse_dolci = {
  caramello: {
    kcal: 382,
    carbs: 97.0,
    protein: 0.0,
    fat: 0.0,
  },
  "caramello salato": {
    kcal: 420,
    carbs: 60.0,
    protein: 1.0,
    fat: 20.0,
  },
  "topping al caramello": {
    kcal: 310,
    carbs: 78.0,
    protein: 0.0,
    fat: 0.0,
  },

  "topping al cioccolato": {
    kcal: 280,
    carbs: 65.0,
    protein: 2.0,
    fat: 2.0,
  },
  "ganache al cioccolato fondente": {
    kcal: 420,
    carbs: 30.0,
    protein: 4.0,
    fat: 32.0,
  },
  "ganache al cioccolato al latte": {
    kcal: 450,
    carbs: 40.0,
    protein: 5.0,
    fat: 30.0,
  },
  "ganache al cioccolato bianco": {
    kcal: 480,
    carbs: 45.0,
    protein: 4.0,
    fat: 32.0,
  },

  "coulis di fragole": {
    kcal: 120,
    carbs: 28.0,
    protein: 0.5,
    fat: 0.2,
  },
  "coulis di frutti rossi": {
    kcal: 110,
    carbs: 25.0,
    protein: 0.6,
    fat: 0.3,
  },
  "coulis di mango": {
    kcal: 130,
    carbs: 30.0,
    protein: 0.5,
    fat: 0.2,
  },

  "sciroppo d'acero": {
    kcal: 260,
    carbs: 67.0,
    protein: 0.0,
    fat: 0.0,
  },
  "sciroppo d'agave": {
    kcal: 310,
    carbs: 76.0,
    protein: 0.0,
    fat: 0.0,
  },
  "sciroppo di datteri": {
    kcal: 310,
    carbs: 75.0,
    protein: 1.0,
    fat: 0.1,
  },
  "sciroppo di cioccolato": {
    kcal: 280,
    carbs: 65.0,
    protein: 2.0,
    fat: 2.0,
  },

  "crema pasticcera": {
    kcal: 215,
    carbs: 30.0,
    protein: 4.0,
    fat: 8.0,
  },
  "crema chantilly": {
    kcal: 280,
    carbs: 20.0,
    protein: 2.0,
    fat: 22.0,
  },
  "crema al mascarpone": {
    kcal: 430,
    carbs: 12.0,
    protein: 4.0,
    fat: 40.0,
  },
  "crema al pistacchio": {
    kcal: 580,
    carbs: 50.0,
    protein: 8.0,
    fat: 38.0,
  },
  "crema al cioccolato spalmabile": {
    kcal: 539,
    carbs: 57.0,
    protein: 6.0,
    fat: 31.0,
  },

  miele: {
    kcal: 304,
    carbs: 82.0,
    protein: 0.3,
    fat: 0.0,
  },
  melassa: {
    kcal: 290,
    carbs: 74.0,
    protein: 0.0,
    fat: 0.0,
  },

  "topping ai frutti di bosco": {
    kcal: 160,
    carbs: 38.0,
    protein: 0.5,
    fat: 0.2,
  },
  "topping al caramello salato": {
    kcal: 350,
    carbs: 60.0,
    protein: 1.0,
    fat: 12.0,
  },
  "topping al cioccolato bianco": {
    kcal: 420,
    carbs: 60.0,
    protein: 3.0,
    fat: 18.0,
  },

  "salsa vaniglia": {
    kcal: 210,
    carbs: 30.0,
    protein: 3.0,
    fat: 8.0,
  },
  "salsa al rum": {
    kcal: 180,
    carbs: 25.0,
    protein: 0.5,
    fat: 0.2,
  },
  "salsa al limone dolce": {
    kcal: 140,
    carbs: 32.0,
    protein: 0.3,
    fat: 0.1,
  },
};

export const salse_da_insalata = {
  "vinaigrette classica": {
    kcal: 380,
    carbs: 2.0,
    protein: 0.2,
    fat: 40.0,
  },
  "vinaigrette al limone": {
    kcal: 220,
    carbs: 3.0,
    protein: 0.3,
    fat: 22.0,
  },
  "vinaigrette balsamica": {
    kcal: 150,
    carbs: 12.0,
    protein: 0.3,
    fat: 10.0,
  },

  "salsa yogurt": {
    kcal: 80,
    carbs: 6.0,
    protein: 3.0,
    fat: 5.0,
  },
  "salsa yogurt light": {
    kcal: 45,
    carbs: 5.0,
    protein: 3.0,
    fat: 1.0,
  },
  "salsa yogurt e cetriolo (tipo tzatziki light)": {
    kcal: 60,
    carbs: 4.0,
    protein: 3.0,
    fat: 3.0,
  },

  "salsa Caesar": {
    kcal: 480,
    carbs: 4.0,
    protein: 4.0,
    fat: 48.0,
  },
  "salsa Caesar light": {
    kcal: 190,
    carbs: 5.0,
    protein: 4.0,
    fat: 12.0,
  },

  "honey mustard": {
    kcal: 260,
    carbs: 30.0,
    protein: 1.0,
    fat: 14.0,
  },
  "mustard vinaigrette": {
    kcal: 210,
    carbs: 6.0,
    protein: 1.0,
    fat: 20.0,
  },

  "salsa tahina per insalata": {
    kcal: 420,
    carbs: 10.0,
    protein: 10.0,
    fat: 38.0,
  },
  "tahina al limone": {
    kcal: 350,
    carbs: 9.0,
    protein: 9.0,
    fat: 30.0,
  },

  "salsa ranch": {
    kcal: 430,
    carbs: 6.0,
    protein: 2.0,
    fat: 44.0,
  },
  "ranch light": {
    kcal: 190,
    carbs: 5.0,
    protein: 2.0,
    fat: 15.0,
  },

  "salsa balsamica cremosa": {
    kcal: 140,
    carbs: 28.0,
    protein: 0.5,
    fat: 1.0,
  },

  "salsa avocado": {
    kcal: 160,
    carbs: 8.0,
    protein: 2.0,
    fat: 14.0,
  },
  "salsa avocado light": {
    kcal: 110,
    carbs: 7.0,
    protein: 2.0,
    fat: 8.0,
  },

  "salsa mediterranea (olio, limone, origano)": {
    kcal: 300,
    carbs: 3.0,
    protein: 0.5,
    fat: 30.0,
  },

  "salsa allo yogurt e senape": {
    kcal: 70,
    carbs: 5.0,
    protein: 3.0,
    fat: 3.0,
  },
};

export const salse_fast_food = {
  "burger sauce (tipo Big Mac)": {
    kcal: 540,
    carbs: 20.0,
    protein: 1.0,
    fat: 50.0,
  },
  "garlic sauce": {
    kcal: 420,
    carbs: 6.0,
    protein: 2.0,
    fat: 42.0,
  },
  ranch: {
    kcal: 430,
    carbs: 6.0,
    protein: 2.0,
    fat: 44.0,
  },
  "ranch light": {
    kcal: 190,
    carbs: 5.0,
    protein: 2.0,
    fat: 15.0,
  },
  "honey mustard": {
    kcal: 260,
    carbs: 30.0,
    protein: 1.0,
    fat: 14.0,
  },
  "mustard bbq": {
    kcal: 180,
    carbs: 25.0,
    protein: 1.0,
    fat: 6.0,
  },
  "bbq sauce": {
    kcal: 165,
    carbs: 40.0,
    protein: 1.0,
    fat: 0.3,
  },
  "smoky bbq": {
    kcal: 180,
    carbs: 42.0,
    protein: 1.0,
    fat: 0.5,
  },
  "sweet & sour": {
    kcal: 190,
    carbs: 45.0,
    protein: 0.5,
    fat: 0.2,
  },
  "sweet chili": {
    kcal: 200,
    carbs: 50.0,
    protein: 0.0,
    fat: 0.0,
  },
  "chipotle mayo": {
    kcal: 620,
    carbs: 4.0,
    protein: 1.0,
    fat: 65.0,
  },
  "spicy mayo": {
    kcal: 600,
    carbs: 3.0,
    protein: 1.0,
    fat: 63.0,
  },
  "sriracha mayo": {
    kcal: 580,
    carbs: 6.0,
    protein: 1.0,
    fat: 58.0,
  },
  "ketchup fast food": {
    kcal: 112,
    carbs: 26.0,
    protein: 1.3,
    fat: 0.1,
  },
  "mayo fast food": {
    kcal: 720,
    carbs: 1.0,
    protein: 1.0,
    fat: 78.0,
  },
  "tartar sauce": {
    kcal: 220,
    carbs: 6.0,
    protein: 1.0,
    fat: 20.0,
  },
  "cocktail sauce (rosa fast food)": {
    kcal: 430,
    carbs: 6.0,
    protein: 1.0,
    fat: 42.0,
  },
  "cheese sauce": {
    kcal: 310,
    carbs: 10.0,
    protein: 8.0,
    fat: 24.0,
  },
  "nacho cheese": {
    kcal: 290,
    carbs: 12.0,
    protein: 7.0,
    fat: 22.0,
  },
  "buffalo sauce": {
    kcal: 110,
    carbs: 1.0,
    protein: 1.0,
    fat: 11.0,
  },
  "hot sauce": {
    kcal: 25,
    carbs: 5.0,
    protein: 0.5,
    fat: 0.1,
  },
  "garlic parmesan sauce": {
    kcal: 520,
    carbs: 4.0,
    protein: 4.0,
    fat: 52.0,
  },
  "truffle mayo": {
    kcal: 640,
    carbs: 3.0,
    protein: 1.0,
    fat: 67.0,
  },
};

export const extra_completo = {
  // ALGHE & SUPERFOOD
  "alga nori": {
    kcal: 35,
    carbs: 5.0,
    protein: 5.8,
    fat: 0.3,
  },
  "alga wakame": {
    kcal: 45,
    carbs: 9.0,
    protein: 3.0,
    fat: 0.6,
  },
  "alga kombu": {
    kcal: 43,
    carbs: 9.6,
    protein: 1.7,
    fat: 0.6,
  },
  spirulina: {
    kcal: 290,
    carbs: 24.0,
    protein: 57.0,
    fat: 8.0,
  },
  clorella: {
    kcal: 410,
    carbs: 23.0,
    protein: 58.0,
    fat: 12.0,
  },
  "erba di grano": {
    kcal: 214,
    carbs: 30.0,
    protein: 20.0,
    fat: 2.0,
  },
  "erba d'orzo": {
    kcal: 216,
    carbs: 34.0,
    protein: 20.0,
    fat: 2.0,
  },
  "cacao crudo": {
    kcal: 228,
    carbs: 57.0,
    protein: 20.0,
    fat: 14.0,
  },
  "bacche di goji": {
    kcal: 349,
    carbs: 77.0,
    protein: 14.0,
    fat: 1.0,
  },
  "semi di canapa": {
    kcal: 553,
    carbs: 9.0,
    protein: 32.0,
    fat: 49.0,
  },
  "maca in polvere": {
    kcal: 325,
    carbs: 71.0,
    protein: 14.0,
    fat: 1.0,
  },
  ashwagandha: {
    kcal: 250,
    carbs: 45.0,
    protein: 3.0,
    fat: 0.5,
  },

  // FERMENTATI
  kimchi: {
    kcal: 15,
    carbs: 2.0,
    protein: 1.1,
    fat: 0.2,
  },
  crauti: {
    kcal: 19,
    carbs: 4.3,
    protein: 0.9,
    fat: 0.1,
  },
  miso: {
    kcal: 199,
    carbs: 26.0,
    protein: 12.0,
    fat: 6.0,
  },
  natto: {
    kcal: 212,
    carbs: 14.0,
    protein: 18.0,
    fat: 11.0,
  },
  kombucha: {
    kcal: 16,
    carbs: 3.0,
    protein: 0.0,
    fat: 0.0,
  },
  tempeh: {
    kcal: 193,
    carbs: 9.0,
    protein: 20.0,
    fat: 11.0,
  },

  // ALTERNATIVE VEGANE AI FORMAGGI
  "formaggio vegano": {
    kcal: 280,
    carbs: 20.0,
    protein: 2.0,
    fat: 22.0,
  },
  "mozzarella vegana": {
    kcal: 250,
    carbs: 20.0,
    protein: 2.0,
    fat: 18.0,
  },
  "spalmabile vegano": {
    kcal: 240,
    carbs: 6.0,
    protein: 3.0,
    fat: 21.0,
  },
  "parmigiano vegano": {
    kcal: 320,
    carbs: 20.0,
    protein: 5.0,
    fat: 25.0,
  },
  "cheddar vegano": {
    kcal: 300,
    carbs: 20.0,
    protein: 3.0,
    fat: 23.0,
  },

  // ALTERNATIVE VEGANE ALLE UOVA
  "uovo vegano liquido": {
    kcal: 45,
    carbs: 2.0,
    protein: 3.0,
    fat: 2.0,
  },
  "farina di ceci (per frittata veg)": {
    kcal: 387,
    carbs: 57.0,
    protein: 22.0,
    fat: 6.7,
  },
  "tofu strapazzato": {
    kcal: 95,
    carbs: 2.0,
    protein: 10.0,
    fat: 5.0,
  },
  aquafaba: {
    kcal: 6,
    carbs: 1.0,
    protein: 0.2,
    fat: 0.0,
  },

  // PRODOTTI FITNESS
  skyr: {
    kcal: 63,
    carbs: 4.0,
    protein: 11.0,
    fat: 0.2,
  },
  "yogurt greco proteico": {
    kcal: 59,
    carbs: 3.6,
    protein: 10.0,
    fat: 0.2,
  },
  "yogurt proteico": {
    kcal: 60,
    carbs: 4.0,
    protein: 10.0,
    fat: 0.2,
  },
  "gelato proteico": {
    kcal: 150,
    carbs: 20.0,
    protein: 12.0,
    fat: 3.0,
  },
  "pudding proteico": {
    kcal: 85,
    carbs: 6.0,
    protein: 12.0,
    fat: 1.0,
  },
  "barretta proteica": {
    kcal: 350,
    carbs: 30.0,
    protein: 25.0,
    fat: 12.0,
  },
  "crema proteica": {
    kcal: 450,
    carbs: 40.0,
    protein: 20.0,
    fat: 25.0,
  },
  "pancake proteico": {
    kcal: 250,
    carbs: 20.0,
    protein: 20.0,
    fat: 8.0,
  },

  // POLVERI & INTEGRATORI
  "proteine whey": {
    kcal: 400,
    carbs: 10.0,
    protein: 80.0,
    fat: 7.0,
  },
  "proteine isolate whey": {
    kcal: 370,
    carbs: 5.0,
    protein: 85.0,
    fat: 3.0,
  },
  "proteine caseine": {
    kcal: 350,
    carbs: 10.0,
    protein: 75.0,
    fat: 3.0,
  },
  "proteine vegane mix": {
    kcal: 380,
    carbs: 8.0,
    protein: 75.0,
    fat: 5.0,
  },
  "proteine di pisello": {
    kcal: 370,
    carbs: 6.0,
    protein: 80.0,
    fat: 5.0,
  },
  "proteine di riso": {
    kcal: 370,
    carbs: 7.0,
    protein: 78.0,
    fat: 4.0,
  },
  "proteine di soia": {
    kcal: 360,
    carbs: 7.0,
    protein: 80.0,
    fat: 4.0,
  },
  "creatina monoidrato": {
    kcal: 0,
    carbs: 0.0,
    protein: 0.0,
    fat: 0.0,
  },
  "bcaa in polvere": {
    kcal: 400,
    carbs: 0.0,
    protein: 100.0,
    fat: 0.0,
  },
  glutammina: {
    kcal: 0,
    carbs: 0.0,
    protein: 0.0,
    fat: 0.0,
  },

  // DOLCIFICANTI SPECIALI
  stevia: {
    kcal: 0,
    carbs: 0.0,
    protein: 0.0,
    fat: 0.0,
  },
};

export const extra_veg_fitness = {
  // Bevande vegetali
  "latte di soia": {
    kcal: 45,
    carbs: 3.0,
    protein: 3.0,
    fat: 2.0,
  },
  "latte di mandorla": {
    kcal: 24,
    carbs: 3.0,
    protein: 0.5,
    fat: 1.0,
  },
  "latte di avena": {
    kcal: 43,
    carbs: 6.5,
    protein: 1.0,
    fat: 1.0,
  },
  "latte di cocco": {
    kcal: 230,
    carbs: 6.0,
    protein: 2.0,
    fat: 24.0,
  },
  "latte di riso": {
    kcal: 47,
    carbs: 9.0,
    protein: 0.3,
    fat: 1.0,
  },
  "latte di anacardi": {
    kcal: 25,
    carbs: 2.0,
    protein: 0.8,
    fat: 1.5,
  },
  "latte di nocciola": {
    kcal: 29,
    carbs: 3.0,
    protein: 0.5,
    fat: 1.5,
  },

  // Proteine vegetali
  tofu: {
    kcal: 76,
    carbs: 1.9,
    protein: 8.0,
    fat: 4.8,
  },
  "tofu affumicato": {
    kcal: 120,
    carbs: 2.0,
    protein: 15.0,
    fat: 6.0,
  },
  tempeh: {
    kcal: 193,
    carbs: 9.0,
    protein: 20.0,
    fat: 11.0,
  },
  seitan: {
    kcal: 143,
    carbs: 5.0,
    protein: 25.0,
    fat: 2.0,
  },
  edamame: {
    kcal: 121,
    carbs: 9.9,
    protein: 11.9,
    fat: 5.2,
  },
  "burger vegetale": {
    kcal: 190,
    carbs: 10.0,
    protein: 15.0,
    fat: 10.0,
  },
  "polpette vegetali": {
    kcal: 180,
    carbs: 12.0,
    protein: 12.0,
    fat: 9.0,
  },
  "affettato vegetale": {
    kcal: 120,
    carbs: 4.0,
    protein: 18.0,
    fat: 3.0,
  },

  // Dolcificanti fitness
  stevia: {
    kcal: 0,
    carbs: 0.0,
    protein: 0.0,
    fat: 0.0,
  },
  eritritolo: {
    kcal: 0,
    carbs: 0.0,
    protein: 0.0,
    fat: 0.0,
  },
  xilitolo: {
    kcal: 240,
    carbs: 100.0,
    protein: 0.0,
    fat: 0.0,
  },
  fruttosio: {
    kcal: 400,
    carbs: 100.0,
    protein: 0.0,
    fat: 0.0,
  },
  destrosio: {
    kcal: 370,
    carbs: 92.0,
    protein: 0.0,
    fat: 0.0,
  },
  maltodestrine: {
    kcal: 380,
    carbs: 95.0,
    protein: 0.0,
    fat: 0.0,
  },
  "sciroppo d'agave": {
    kcal: 310,
    carbs: 76.0,
    protein: 0.0,
    fat: 0.0,
  },

  // Polveri proteiche
  "proteine whey": {
    kcal: 400,
    carbs: 10.0,
    protein: 80.0,
    fat: 7.0,
  },
  "proteine isolate whey": {
    kcal: 370,
    carbs: 5.0,
    protein: 85.0,
    fat: 3.0,
  },
  "proteine caseine": {
    kcal: 350,
    carbs: 10.0,
    protein: 75.0,
    fat: 3.0,
  },
  "proteine vegane mix": {
    kcal: 380,
    carbs: 8.0,
    protein: 75.0,
    fat: 5.0,
  },
  "proteine di pisello": {
    kcal: 370,
    carbs: 6.0,
    protein: 80.0,
    fat: 5.0,
  },
  "proteine di riso": {
    kcal: 370,
    carbs: 7.0,
    protein: 78.0,
    fat: 4.0,
  },
  "proteine di soia": {
    kcal: 360,
    carbs: 7.0,
    protein: 80.0,
    fat: 4.0,
  },

  // Alimenti fitness
  "fiocchi di latte light": {
    kcal: 98,
    carbs: 3.0,
    protein: 11.0,
    fat: 4.0,
  },
  "yogurt greco 0%": {
    kcal: 59,
    carbs: 3.6,
    protein: 10.0,
    fat: 0.2,
  },
  "barretta proteica": {
    kcal: 350,
    carbs: 30.0,
    protein: 25.0,
    fat: 12.0,
  },
  "pancake proteico": {
    kcal: 250,
    carbs: 20.0,
    protein: 20.0,
    fat: 8.0,
  },
  "burro di arachidi": {
    kcal: 588,
    carbs: 20.0,
    protein: 25.0,
    fat: 50.0,
  },
  "burro di mandorle": {
    kcal: 614,
    carbs: 19.0,
    protein: 21.0,
    fat: 55.0,
  },
  "crema proteica": {
    kcal: 450,
    carbs: 40.0,
    protein: 20.0,
    fat: 25.0,
  },

  // Altri alimenti vegani
  hummus: {
    kcal: 166,
    carbs: 14.0,
    protein: 8.0,
    fat: 10.0,
  },
  falafel: {
    kcal: 333,
    carbs: 31.0,
    protein: 13.0,
    fat: 17.0,
  },
  "latte di cocco light": {
    kcal: 45,
    carbs: 2.0,
    protein: 0.5,
    fat: 3.0,
  },
};

export const legumi = {
  "ceci secchi": {
    kcal: 364,
    carbs: 61.0,
    protein: 19.0,
    fat: 6.0,
  },
  "ceci cotti": {
    kcal: 164,
    carbs: 27.0,
    protein: 9.0,
    fat: 2.6,
  },

  "lenticchie secche": {
    kcal: 352,
    carbs: 60.0,
    protein: 25.0,
    fat: 1.1,
  },
  "lenticchie cotte": {
    kcal: 116,
    carbs: 20.0,
    protein: 9.0,
    fat: 0.4,
  },

  "fagioli borlotti secchi": {
    kcal: 335,
    carbs: 60.0,
    protein: 23.0,
    fat: 1.2,
  },
  "fagioli borlotti cotti": {
    kcal: 140,
    carbs: 25.0,
    protein: 9.0,
    fat: 0.5,
  },

  "fagioli cannellini secchi": {
    kcal: 333,
    carbs: 61.0,
    protein: 23.0,
    fat: 0.8,
  },
  "fagioli cannellini cotti": {
    kcal: 138,
    carbs: 25.0,
    protein: 9.0,
    fat: 0.5,
  },

  "fagioli neri": {
    kcal: 339,
    carbs: 62.0,
    protein: 21.0,
    fat: 0.9,
  },
  "fagioli rossi kidney": {
    kcal: 337,
    carbs: 60.0,
    protein: 24.0,
    fat: 0.8,
  },

  "piselli secchi": {
    kcal: 352,
    carbs: 63.0,
    protein: 24.0,
    fat: 1.0,
  },
  "piselli cotti": {
    kcal: 81,
    carbs: 14.0,
    protein: 5.4,
    fat: 0.4,
  },

  "soia secca": {
    kcal: 446,
    carbs: 30.0,
    protein: 36.0,
    fat: 20.0,
  },
  "soia edamame": {
    kcal: 121,
    carbs: 9.9,
    protein: 11.9,
    fat: 5.2,
  },

  "fave fresche": {
    kcal: 88,
    carbs: 18.0,
    protein: 7.0,
    fat: 0.7,
  },
  "fave secche": {
    kcal: 343,
    carbs: 58.0,
    protein: 26.0,
    fat: 1.5,
  },

  lupini: {
    kcal: 114,
    carbs: 10.0,
    protein: 16.0,
    fat: 2.0,
  },

  cicerchie: {
    kcal: 316,
    carbs: 49.0,
    protein: 22.0,
    fat: 2.0,
  },

  "mung bean (fagioli mung)": {
    kcal: 347,
    carbs: 63.0,
    protein: 24.0,
    fat: 1.2,
  },
  azuki: {
    kcal: 329,
    carbs: 63.0,
    protein: 20.0,
    fat: 0.5,
  },
};

export const frutta_secca_semi = {
  mandorle: {
    kcal: 579,
    carbs: 22.0,
    protein: 21.0,
    fat: 50.0,
  },
  nocciole: {
    kcal: 628,
    carbs: 17.0,
    protein: 15.0,
    fat: 61.0,
  },
  noci: {
    kcal: 654,
    carbs: 14.0,
    protein: 15.0,
    fat: 65.0,
  },
  "noci pecan": {
    kcal: 691,
    carbs: 14.0,
    protein: 9.0,
    fat: 72.0,
  },
  "noci brasiliane": {
    kcal: 659,
    carbs: 12.0,
    protein: 14.0,
    fat: 67.0,
  },
  "noci macadamia": {
    kcal: 718,
    carbs: 14.0,
    protein: 8.0,
    fat: 76.0,
  },

  arachidi: {
    kcal: 567,
    carbs: 16.0,
    protein: 26.0,
    fat: 49.0,
  },
  "arachidi tostate": {
    kcal: 590,
    carbs: 18.0,
    protein: 25.0,
    fat: 50.0,
  },

  pistacchi: {
    kcal: 562,
    carbs: 28.0,
    protein: 20.0,
    fat: 45.0,
  },
  anacardi: {
    kcal: 553,
    carbs: 30.0,
    protein: 18.0,
    fat: 44.0,
  },
  pinoli: {
    kcal: 673,
    carbs: 13.0,
    protein: 14.0,
    fat: 68.0,
  },

  "semi di zucca": {
    kcal: 559,
    carbs: 11.0,
    protein: 30.0,
    fat: 49.0,
  },
  "semi di girasole": {
    kcal: 584,
    carbs: 20.0,
    protein: 21.0,
    fat: 51.0,
  },
  "semi di sesamo": {
    kcal: 573,
    carbs: 23.0,
    protein: 18.0,
    fat: 50.0,
  },
  "semi di lino": {
    kcal: 534,
    carbs: 29.0,
    protein: 18.0,
    fat: 42.0,
  },
  "semi di chia": {
    kcal: 486,
    carbs: 42.0,
    protein: 17.0,
    fat: 31.0,
  },
  "semi di papavero": {
    kcal: 525,
    carbs: 28.0,
    protein: 18.0,
    fat: 42.0,
  },

  uvetta: {
    kcal: 299,
    carbs: 79.0,
    protein: 3.1,
    fat: 0.5,
  },
  "uvetta sultanina": {
    kcal: 302,
    carbs: 80.0,
    protein: 3.0,
    fat: 0.5,
  },

  datteri: {
    kcal: 282,
    carbs: 75.0,
    protein: 2.5,
    fat: 0.4,
  },
  "datteri medjool": {
    kcal: 277,
    carbs: 75.0,
    protein: 1.8,
    fat: 0.2,
  },

  "fichi secchi": {
    kcal: 249,
    carbs: 64.0,
    protein: 3.3,
    fat: 0.9,
  },
  "albicocche secche": {
    kcal: 241,
    carbs: 63.0,
    protein: 3.4,
    fat: 0.5,
  },
  "prugne secche": {
    kcal: 240,
    carbs: 64.0,
    protein: 2.2,
    fat: 0.4,
  },
  "mirtilli rossi secchi": {
    kcal: 308,
    carbs: 82.0,
    protein: 0.1,
    fat: 1.4,
  },

  "cocco essiccato": {
    kcal: 660,
    carbs: 24.0,
    protein: 7.0,
    fat: 64.0,
  },
  "banana essiccata": {
    kcal: 346,
    carbs: 88.0,
    protein: 3.9,
    fat: 1.2,
  },
  "mele essiccate": {
    kcal: 243,
    carbs: 65.0,
    protein: 0.9,
    fat: 0.3,
  },
  "pere essiccate": {
    kcal: 262,
    carbs: 70.0,
    protein: 1.5,
    fat: 0.6,
  },

  "mix frutta secca": {
    kcal: 520,
    carbs: 28.0,
    protein: 15.0,
    fat: 40.0,
  },
};

export const nutritionDB = {
  pasta_riso_farine,
  carni_salumi,
  pesce_frutti_di_mare,
  latticini_formaggi,
  verdure,
  frutta_fresca,
  frutta_secca_semi,
  dolci_creme,
  legumi,
  oli_grassi,
  extra_veg_fitness,
  extra_completo,
  salse_base,
  salse_italiane,
  salse_asiatiche,
  salse_da_insalata,
  salse_dolci,
  salse_fast_food,
};

export type FoodItem = {
  name: string;
  category: string;
  kcal: number;
  carbs: number;
  protein: number;
  fat: number;
};

export const flatNutritionDB: FoodItem[] = Object.entries(nutritionDB).flatMap(
  ([category, items]) =>
    Object.entries(items as Record<string, any>).map(([name, data]) => ({
      name,
      category,
      kcal: data.kcal,
      carbs: data.carbs ?? 0,
      protein: data.protein ?? 0,
      fat: data.fat ?? 0,
    })),
);

export const calculateMacros = (foodName: string, grams: number) => {
  const item = flatNutritionDB.find(
    (f) => f.name.toLowerCase() === foodName.toLowerCase(),
  );
  if (!item) return null;

  const factor = grams / 100;

  return {
    kcal: item.kcal * factor,
    carbs: item.carbs * factor,
    protein: item.protein * factor,
    fat: item.fat * factor,
  };
};
